...
| DELETE | `/restrooms/{id}?force=` | 删除；无关联数据时物理删除；仅有已闭环历史数据时需 `force=true` 归档台账并保留历史，未闭环问题始终拦截 |
| GET | `/restrooms/{id}/delete-impact` | 删除前影响面：巡查/问题/流水/附件数量、阻断原因、保留与移出范围 |
...
| GET | `/stats/monthly-report?month=YYYY-MM` | 月度历史口径报表及归档公厕造成的当前看板差异说明 |
...
- **删除边界与保护**：删除公厕前必须先读取影响面。无巡查、无问题时才允许物理删除；只要存在巡查或已闭环问题，`force=true` 也只是把公厕档案归档：巡查、问题、整改流水和附件链接全部保留，历史统计与月报仍可追溯。未闭环问题不会被强制删除，必须先完成整改、验收关闭或作废关闭。归档与删除在一个数据库事务内提交，失败整体回退，并写入删除审计快照（数量、原因、操作人、影响面）。
- **统计口径**：当前运行看板只统计在册公厕，问题状态/分类/严重程度等历史分布保留归档公厕的数据；区域运行单列“归档保留”，月度报表提供归档数量、保留数据数量和差异解释，避免月报数字无声减少。
