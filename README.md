...
| DELETE | `/restrooms/{id}?force=&operator=&reason=` | 删除；有关联数据时需 `force=true`，未闭环问题会阻断删除；删除写入审计并返回随删摘要 |
| GET | `/restrooms/{id}/deletion-impact` | 删除影响面评估：随删的巡查/问题/整改流水/附件数量与阻断原因 |
| GET | `/restrooms/deletion-logs` | 删除审计分页查询（随删数据规模、问题状态/分类分布、档案快照、操作人、原因） |
...
- **删除边界与审计**：删除公厕前可调用 `deletion-impact` 评估影响面。删除边界：公厕档案、巡查记录、已闭环问题及其整改流水与附件随公厕一并删除；**未闭环问题（待整改/整改中/待验收）属于进行中的整改工单，即使 `force=true` 也拒绝删除**，须先闭环或关闭。删除时把随删数据的规模、问题状态/分类分布与档案快照写入删除审计（`deletion-logs` 可查），审计写入与级联删除在同一事务，任一步失败整体回退；看板「在册公厕」卡片展示累计删除数，报表数字的变化可对照审计解释。
...
- 后端 `pytest`：8 个用例全部通过；`/health`、台账/巡查/问题/统计/字典接口均返回预期数据。
